...
export default class Cards extends React.Component {
  ...
  componentDidMount() {
      ...
      if (currLocation !== 'http://localhost:3000/cards')
        ...
        console.log(pagenum);
         var url = 'https://projectpokemon.org/images/normal-sprite/charizard.gif';
         fetch(url)
            ...
            <Card>
               <CardHeader
                ...
                //  avatar="https://cdn.intra.42.fr/users/oseng.jpg"
                 avatar="https://projectpokemon.org/images/normal-sprite/charizard.gif"
               />
               <CardMedia
                 overlay={<span title={this.state.myJson[0].name} subtitle={this.state.myJson[0].id} />}
               >
                 <img src={this.state.myJson[0].thumbnail.path + '.' + this.state.myJson[0].thumbnail.extension} alt="" />
               </CardMedia>
               <span title={this.state.myJson[0].name} subtitle={this.state.myJson[0].id} />
               <CardContent>
               ...
               { this.state.myJson[0].urls.map((dynamicData, key) =>
                 <span key={key} style={styles.chip} >{dynamicData.url}</span>
               )}
               </CardActions>
             </Card>
        </div>
